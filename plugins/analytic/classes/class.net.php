...
class Http {
	...
	/**
	 * @var boolean True to verify the SSL cert, false otherwise
	 */
	...
	/**
	 * @var boolean True to enable persistent connections, false to reset the connection for each request
	 */
	 ...
	 *
	 * @param boolean $persist True to persist, false not to.
	 */
	 ...
	 *
	 * @param mixed $params An array of key/value pairs, or a string of the format key=value&...
	 * @param array $files A multi-dimensional array of files in the format of [0]=>array("name"=>"Name of file", "file"=>"File path")
	 * @return string The response from the URL
	 * @see NetHttp::responseCode()
	 */
	public function post($url, $params=null, $files=null) {
		if (is_array($files)) {
			...
			for ($i=0; $i<$num_files; $i++)
				$params[$files[$i]['name']]  = "@" . $files[$i]['file'];
			unset($num_files);
		}
	 ...
	 * @param boolean $override True to override the existing value, otherwise will only set the value if not already set
	 */
