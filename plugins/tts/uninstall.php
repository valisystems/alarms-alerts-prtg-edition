...
<html lang="en">
<head>
	<title>Uninstallation - TTS Plugin</title>
	<meta charset="utf-8">
...
<body>
    ...
    <div class="container">
        <div class="row">
        	<div class="col-md-12">
            <h3>Uninstallation - TTS Plugin</h3>
            ...
            <?php if (isset($_POST['uninstall']))
            {
                ...
                if ($rows)
                {
                    $jakdb->query('DELETE FROM '.DB_PREFIX.'plugins WHERE name = "TTS"');
                    ...
                    $jakdb->query('DELETE FROM '.DB_PREFIX.'pluginhooks WHERE product = "tts"');

                    $jakdb->query('DELETE FROM '.DB_PREFIX.'setting WHERE product = "tts"');

                    $jakdb->query('ALTER TABLE '.DB_PREFIX.'usergroup
                    				DROP `tts`, DROP `ttspost`, DROP `ttspostdelete`,
                    				DROP `ttspostapprove`');

                    $jakdb->query('DROP TABLE '.DB_PREFIX.'tts');
