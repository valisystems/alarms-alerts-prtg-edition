...
<nav class="navbar navbar-<?php echo $jkv["navbarbw_mosaic_tpl"];?>">
	<button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#shop-nav" aria-expanded="false">
	      ...
	      <div class="collapse navbar-collapse" id="shop-nav">
			...
			<li<?php if ($page3 == "") echo ' class="active"';?>><a href="<?php echo $backtoshop;?>" data-name="all"><?php echo $tlec["shop"]["m52"];?></a></li>

<?php if (isset($JAK_SHOP_CAT) && is_array($JAK_SHOP_CAT)) foreach($JAK_SHOP_CAT as $mv) { if ($mv["catparent"] == '0') { ?>

<li<?php if (!empty($page3) && $page3 == $mv["varname"]) echo ' class="active"';?>><a href="<?php echo $mv['parseurl'];?>" data-name="<?php echo $mv["varname"];?>"><?php if ($mv["catimg"]) { ?><i class="fa <?php echo $mv["catimg"];?> fa-fw"></i> <?php } ?><?php echo $mv["name"];?></a>
...
<li<?php if (!empty($page3) && $page3 == $mz["varname"])  echo ' class="active"';?>><a href="<?php echo $mz['parseurl'];?>" data-name="<?php echo $mz["varname"];?>"><?php if ($mz["catimg"]) { ?><i class="fa <?php echo $mz["catimg"];?> fa-fw"></i> <?php } echo $mz["name"];?></a></li>
<?php } } ?>
</ul>
</li>
<?php } else { ?>
...
<div class="drag-cart drop-here" id="shopping-cart">
	<div class="row">
		...
		<div class="col-md-1" id="trashcontainer">
			<a href="#" id="trash"><i class="fa fa-trash-o fa-3x"></i></a>
		</div>
	</div>
</div>
<div class="row shop-mosaic-items">
