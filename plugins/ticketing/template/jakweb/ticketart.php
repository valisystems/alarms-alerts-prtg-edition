...
<div id="printdiv">
	<div class="jak-post jak-single-post">
		<div class="single-post-title">
			<h3><?php if (isset($JAK_ST_OPT) && is_array($JAK_ST_OPT)) foreach($JAK_ST_OPT as $sto) { if ($sto["id"] == $row['typeticket'] && $sto["img"]) { ?>
				    <a href="<?php echo $sto["parseurl"];?>" title="<?php echo $sto["name"];?>"><i class="fa <?php echo $sto["img"];?>"></i></a>
			<?php } } ?> <?php echo $TICKET_TITLE;?></h3>
		</div>
		<div class="single-post-info">
			<i class="fa fa-clock-o"></i> <?php echo $PAGE_TIME;?> <i class="fa fa-eye"></i> <?php echo $row['hits'];?><br>
			<strong><?php echo $tlt["st"]["t35"];?>:</strong> <span class="label label-default"><?php echo $row["catname"];?></span> <strong><?php echo $tlt["st"]["t14"];?>:</strong> <span class="label label-default"><?php echo $row["toption"];?></span> <?php echo '<strong>'.$tlt["st"]["t28"].':</strong> '.$TICKER_STATUS;?> <?php echo '<strong>'.$tlt["st"]["t32"].':</strong> '.$TICKER_RESO;?> <?php echo '<strong>'.$tlt["st"]["t17"].'</strong>'.(JAK_ASACCESS ? '<a href="'.BASE_URL.'admin/index.php?p=user&sp=edit&ssp='.$row["userid"].'">'.$row["username"].'</a>' : $row["username"]);?>
		</div>
