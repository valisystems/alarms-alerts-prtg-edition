...
<div class="alert alert-danger fade in">
  <button type="button" class="close" data-dismiss="alert">×</button>
  <h4><?php if ($page2 == "e") { $tl["errorpage"]["sql"]; } elseif ($page2 == "epc") { echo $tl["errorpage"]["pluginc"]; } elseif ($page2 == "ene") { echo $tl["errorpage"]["not"]; } else { echo $tl["errorpage"]["cat"]; } ?></h4>
</div>
<?php } ?>
...
<i title="<?php echo $tlgal["gallery"]["q9"];?>" class="fa fa-sticky-note-o"></i>
<i title="<?php echo $tlgal["gallery"]["q4"];?>" class="fa fa-picture-o"></i>
...
$(document).ready(function() {
	...
	$(".save-menu-plugin").on("click", function() {
		...
		var request = $.ajax({
		  url: "index.php?p=gallery&amp;sp=categories",
		  type: "POST",
		  data: serialized,
		  dataType: "json",
		  cache: false
	...
	$(".watermark").click(function() {
		$(this).closest(".actions").find(".loader").show();
	});
