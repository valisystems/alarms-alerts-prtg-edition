...
<div id="slider" style="width: <?php echo $rowls['lswidth'];?>; height: <?php echo $rowls['lsheight'];?>; margin: 0px auto;">
        
	<?php
        
    	$lsl = $jakdb->query('SELECT id, slide2d, slide3d, timeshift, lsdeep, slidedirection, slidedelay, durationin, durationout, easingin, easingout, delayin, delayout FROM '.DB_PREFIX.'slider_layers WHERE lsid = "'.$rowls['id'].'" AND layer = 0 ORDER BY id ASC LIMIT 10');
        ...
        $lsp = $jakdb->query('SELECT lsposition, lsmove, lsstyle, lslink, lspath, imgdirection, durationin, durationout, easingin, easingout, delayin, delayout, parallaxin, parallaxout FROM '.DB_PREFIX.'slider_layers WHERE lsid = '.$rowls['id'].' AND layer = "'.$rowlsl['id'].'" ORDER BY id ASC LIMIT 20');
