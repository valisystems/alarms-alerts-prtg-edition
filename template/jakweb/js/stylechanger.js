...
$(document).ready(function(){
	...
	$('#jak_stylebutton').on('click',function(e) {
		e.preventDefault();
	    slide_out_box('#jak_stylechanger')
	});
	
	// Navbar Fixed or Float
	$(document).on('change', '#navbarstyle', function(e) {
		e.preventDefault();
		if ($('.navbar').hasClass("navbar-fixed-top")) {
		    $('#sb-site').css("margin-top", "0");
		    $('.navbar').css("margin-bottom", "0").removeClass("navbar-fixed-top");
		} else {
			$('#sb-site').css("margin-top", "50px");
			$('.navbar').css("margin-bottom", "auto").addClass("navbar-fixed-top");
		}
	});
	
	// Footer big or small
	$(document).on('change', '#footerstyle', function(e) {
		e.preventDefault();
		$('.footer').toggleClass("hidden");
		if ($('.footer-small').hasClass("hidden")) {
			$('.footer-block-big').fadeIn();
		} else {
			$('.footer-block-big').fadeOut();
		}
	});
	...
	// Boxed or Fluid
	$(document).on('change', '#tplstyle', function(e) {
		e.preventDefault();
		boxed = $(this).val();
		
		$("body").removeClass("boxed-layout container");
		$('.bgboxed').addClass("hidden");
		if (boxed) {
			$("body").addClass("boxed-layout container");
			$('.bgboxed').removeClass("hidden");
		}
		
	});
	
	// Dark or Light
	$(document).on('change', '#tplcolor', function() {
		tplcolor = $(this).val();
		
		//$('.tpljakweb').remove();
		if (tplcolor == "dark") {
			var csscolor = "screendark.css";
		} else {
	...
	$(document).on('change', '#gFont', function() {
		...
		if (tplgfont != "NonGoogle") {
			$("head").append('<link rel="stylesheet" href="http://fonts.googleapis.com/css?family='+tplgfont+':regular,italic,bold,bolditalic" type="text/css" class="scgoogle" />');
			$('h1, h2, h3, h4, h5, h6').css("font-family", tplgfont.replace("+", " "));
		}
	...
	$(document).on('change', '#cFont', function() {
		...
		$('.scgoogle').remove();
		if (tplfont != "NonGoogle") {
			$('body, code, input[type="text"], textarea').css("font-family", tplfont);
		}
		if (tplgfont != "NonGoogle") {
			$("head").append('<link rel="stylesheet" href="http://fonts.googleapis.com/css?family='+tplgfont+':regular,italic,bold,bolditalic" type="text/css" class="scgoogle" />');
			$('h1, h2, h3, h4, h5, h6').css("font-family", tplgfont.replace("+", " "));
	...
	// Boxed Background
	$('.tplboxbgcolor').colorpicker({format: "hex"}).on('changeColor.colorpicker', function(e){
	  if ($('.tplboxbgcolor').val()) $('.boxed-layout').css("background", e.color.toHex());
	});
	
	$('.colorbg').colorpicker({format: "hex"}).on('changeColor.colorpicker', function(e){
	  if ($('.colorbg').val()) $('#sb-site').css("background", e.color.toHex());
	});
	
	$('.colorn').colorpicker().on('changeColor.colorpicker', function(e){
	  if ($('.colorn').val()) $('.navbar').css("background", e.color);
	});
	
	$('.colornt').colorpicker({format: "hex"}).on('changeColor.colorpicker', function(e){
	  if ($('.colornt').val()) $('.navbar a').css("color", e.color.toHex());
	});
	
	$('.colorntb').colorpicker({format: "hex"}).on('changeColor.colorpicker', function(e){
	  if ($('.colorntb').val()) $('.navbar-default .navbar-nav > .active > a').css("background", e.color.toHex());
	});
	
	$('.colornts').colorpicker({format: "hex"}).on('changeColor.colorpicker', function(e){
	  if ($('.colornts').val()) $('ul.nav-main ul.dropdown-menu').css("background", e.color.toHex());
	});
	
	// Logo
	$('#sclogo').on("change paste keyup", function() {
		var src = $(this).val();
		$('#main-logo').attr("src", src);
	});
	
	// section content box 1
	$(document).on('keyup', '#jakEditor1', function(e) {
		e.preventDefault();
		$('#section-content').html($(this).val());
	});
	
	// section content box 2
	$(document).on('keyup', '#jakEditor2', function(e) {
		e.preventDefault();
		$('#section-content2').html($(this).val());
	});
	
	// section content box 3
	$(document).on('keyup', '#jakEditor3', function(e) {
		e.preventDefault();
		$('#section-content3').html($(this).val());
	});
	
	// section background color
	$('.colorc').colorpicker().on('changeColor.colorpicker', function(e){
	  $('.section-white').css("background", e.color);
	});
	
	// section h3 color
	$('.colorct').colorpicker({format: "hex"}).on('changeColor.colorpicker', function(e){
	  $('.section-white h3').css("color", e.color.toHex());
	});
	
	// Section Hide
	$('#jak_sectionhide').on('click',function(e) {
		e.preventDefault();
	    if ($("#footer-section").hasClass("hidden")) {
	    	$("#footer-section").removeClass("hidden");
	        $('#hide-section').val("1");
	        $('#jak_sectionhide').html("Hide");
	    } else {
	    	$("#footer-section").addClass("hidden");
	        $('#hide-section').val("0");
	        $('#jak_sectionhide').html("Show");
	    }
	});
	
	// footer content box 1
	$(document).on('keyup', '#fcont', function(e) {
		e.preventDefault();
		$('#footer-content').html($(this).val());
	});
	
	// footer content box 2
	$(document).on('keyup', '#fcont2', function(e) {
		e.preventDefault();
		$('.footer-content2').html($(this).val());
	});
	
	// footer content box 3
	$(document).on('keyup', '#fcont3', function(e) {
		e.preventDefault();
		$('#footer-content3').html($(this).val());
	});
	
	// footer color
	$('.colorf').colorpicker().on('changeColor.colorpicker', function(e){
	  $('.footer').css("background", e.color);
	});
	
	// footer h3 color
	$('.colorft').colorpicker({format: "hex"}).on('changeColor.colorpicker', function(e){
	  $('.footer h3').css("color", e.color.toHex());
	});
	
	// footer text color
	$('.colorfte').colorpicker({format: "hex"}).on('changeColor.colorpicker', function(e){
	  $('.footer').css("color", e.color.toHex());
	});
	
	$('#sctabs a').click(function (e) {
	  e.preventDefault()
	  $(this).tab('show')
	})
...
function slide_out_box(el) {
	
	if ($(el).css('left') == '0px') {
	    $(el).animate({left: '-500px'}, 300);
	} else {
